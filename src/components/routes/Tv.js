...
const Tv= ()=>  {
    ...
    const getMovie = async() => {
        try {
            ...
            const url = (`https://api.themoviedb.org/3/tv/${params.type}?api_key=${apiKey}&page=${currentPage}`)
            const movieData = await fetch(url)
            const fetchedData = await movieData.json()
            console.log("tv",fetchedData)
            if (fetchedData) {
  ...
  return (
    <>
            ...
            <div class="relative inline-block text-left">
                ...
                <div class={`${dropdown} absolute right-0 z-10 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1`}>
                    <div class="py-1" role="none">
                        <Link to="/tv/airing_today" class="text-gray-700 hover:bg-gray-700 hover:text-white block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-0">Airing_today</Link>
                        <Link to="/tv/on_the_air" class="text-gray-700 hover:bg-gray-700 hover:text-white block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-1">On_The_Air</Link>
                        <Link to="/tv/popular" class="text-gray-700 hover:bg-gray-700 hover:text-white block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-2">Popular</Link>
                        <Link to="/tv/top_rated" class="text-gray-700 hover:bg-gray-700 hover:text-white block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-3">Top Rated</Link>
                    </div>
                </div>
            </div>
        </div>
        <div className='w-[80%] mx-auto my-10 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4'>
            {movies?.map((tv)=>{
            return  <NavLink to={`/details/tv/${tv.id}`} className="flex mx-auto justify-between " key={tv.id}>
                        <Card key={tv.id} title={tv.original_name} image={`https://image.tmdb.org/t/p/w500${tv.poster_path}`} />
                    </NavLink>
            })}
        </div>
        ...
        activeClassName={'bg-sky-600 p-2'}
        />
    </>
  )
}
